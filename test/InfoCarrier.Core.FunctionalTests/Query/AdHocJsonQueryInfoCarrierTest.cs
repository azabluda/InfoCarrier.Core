...
/// <summary>
///     <c>AdHocJsonQueryRelationalTestBase</c> on ADR-009 <b>Tier B</b> — the last spec base the
///     compliance test listed (B3d, C10, adopted in C82).
/// </summary>
/// <remarks>
///     <para>
///         B3d priced this at <em>"626 + 322 lines of relational mirror and seven abstract seeds
///         only EF's relational classes implement"</em>, and C10 re-checked it and agreed. Both
///         halves of that price are gone. The 626 + 322 was the cost of <b>not referencing</b>
///         <c>Microsoft.EntityFrameworkCore.Relational.Specification.Tests</c>, which ADR-013 now
///         does — so the model configuration is inherited rather than mirrored. And the seven
///         abstract seeds are seven raw-SQL <c>INSERT</c>s, copied verbatim from
///         <c>AdHocJsonQuerySqliteTest</c> because the backing store <em>is</em> SQLite.
///     </para>
///     <para>
///         The one thing that is not a copy is where the SQL runs. A seed receives the
///         <b>client's</b> context, which has no database and no <c>ExecuteSql</c>; these rows
///         describe what the store holds, so they are executed against the backend directly
///         (<see cref="ExecuteOnBackendAsync" />). That is the same reasoning
///         <c>GraphUpdatesInfoCarrierTest.ReseedAsync</c> uses: a seed that went through the client
///         would make every test's setup depend on the thing under test.
///     </para>
///     <para>
///         The corpus is deliberately malformed JSON — junk properties, missing scalars, missing
///         required navigations, shadow properties, nulls where objects belong. It is the one base
///         that asks what this provider does with a document the model does not fully describe.
///     </para>
/// </remarks>
public class AdHocJsonQueryInfoCarrierTest(NonSharedFixture fixture)
    : AdHocJsonQueryRelationalTestBase(fixture)
{
    ...
    /// <summary>
    ///     Runs a seed statement against the backing store rather than through the wire.
    /// </summary>
    /// <remarks>
    ///     The client context these seeds are handed has no database at all, so
    ///     <c>Database.ExecuteSqlAsync</c> is not available on it — and would be the wrong thing
    ///     anyway: these rows are a statement about what the store already contains, not about
    ///     anything this provider should be asked to write.
    /// </remarks>
