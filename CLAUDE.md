...
| `eng/ef-sql-diff.py <server-sql.log> [--efcore <path>] [--limit N] [--extras] [--survey]` | The comparing half of the script above, on a log that already exists. Reports LITERAL, PARAMETER, VALUE and STRUCTURAL, counted per test; **this row read EXTRA READ, EXTRA WRITE and ORDER until 2026-09-20**, three kinds the ordered-subsequence rewrite of #119 had already deleted. **`--extras` reads what EF does not assert** (the test's own arrange and verify, and any round trip of ours that EF's window never sees), grouped by shape, unbounded reads first. **`--survey` reads EVERY statement in a log** and needs no reference at all, which is the mode for a tier upstream gives nothing to compare with. |
