...
Use the `--version` option, or `--prerelease`. `InfoCarrier.Core` has been on nuget.org since 1.0
and its newest stable release is `3.1.1`, built for EF Core 3.1, so an unversioned install resolves
to that one instead. It does not fail and it does not warn. With Central Package Management, pin the
version in `Directory.Packages.props` and give both packages the same one.
