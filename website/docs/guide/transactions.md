...
A client transaction is a **real transaction on the server's database**. `BeginTransaction` opens
one there and hands the client a token; every request the client makes afterwards names that token,
