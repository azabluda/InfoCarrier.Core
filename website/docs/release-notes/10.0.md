...
Synchronous `DbContext` calls still work. `SaveChanges` blocks on the async path, so the calling
thread waits for the round trip. Every await inside the provider uses `ConfigureAwait(false)`, so
it does not deadlock on a UI synchronization context, but a WPF or WinForms caller on the UI thread
freezes the window until the answer arrives. Call the async API from a UI thread.
