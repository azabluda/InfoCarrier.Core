...
The serializer applies a size bound to what it will **deserialize**. It is a security control, so it
is default-on for the direction that matters:
...
Standard EF Core logging, and worth turning on while you are learning what crosses the wire:
...
They work, and they run on the client. But remember that the server executes your query against
**its own** model: a global query filter defined on the server's model is applied there and cannot
be bypassed from a client, which is what makes filters the right place for authorization.
