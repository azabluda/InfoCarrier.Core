...
**ARCHIVED 2026-08-24, when M8 closed. Never edited again.** The relative links below were
repointed one directory deeper by the move itself and nothing else was touched.

This was the rolling checkbox detail for M8. Closed milestones are archived here:
...
- [x] **N26. The README, cut to what a README is for, and `CONTRIBUTING.md` to hold the rest.** `<this commit>`

      The README was **262 lines and 1,645 words**, with the pitch made three times, the security
      boundary explained twice at length, and `docs/limitations.md` pointed at from three places. It
      is now **132 lines and 528 words**, and `CONTRIBUTING.md` is new.

      **The shape was decided by measurement, not by taste.** Twelve well-known .NET repositories
      were fetched and counted:

      | | lines | words |
      |---|---|---|
      | Newtonsoft.Json | 12 | 26 |
      | xunit | 31 | 250 |
      | quartznet | 32 | 124 |
      | FluentValidation | 73 | 294 |
      | AutoMapper | 93 | 368 |
      | Humanizer | 102 | 354 |
      | serilog | 119 | 835 |
      | MediatR | 128 | 489 |
      ...
      **Of the twelve, zero mention a test count, zero have a Security section, and zero have an
      architecture or "how it works" section.** Only five mention building at all, and each of those
      is one line pointing at `BUILDING.md` or `CONTRIBUTING.md`. The documentation link is near the
      top: line 7 in FluentValidation, line 10 in quartznet, the first section in MassTransit, and
      the whole of Newtonsoft.Json's README is a link list. **None of them explains what its
      documentation site contains.**

      So: the suite numbers, the two-package cost table, the security paragraphs, the comparison
      with the 1.0–3.1 line and the ten-row internal docs index all left the README. Each was
      checked to exist elsewhere before it was cut, and the site pages that now carry them were
      confirmed present in `mkdocs.yml`'s nav so the links cannot 404. The `3.1.1` install trap
      still appears in `docs/nuget-readme.md` and on two site pages, which are where a stranger
      meets it.

      `CONTRIBUTING.md` takes the build, both test projects, the never-skip-a-spec-test rule, the
      `CI=true` gate, the which-gate-to-run table, `eng/docs-serve.sh`, `eng/measure.sh`,
      `eng/trim-ratchet.sh`, and the internal docs index.

      **The prose was rewritten with the `humanizer` skill** (Wikipedia's *Signs of AI writing*).
      Em dashes went from **25 to 0** outside code blocks, bold spans from **38 to 3**. The two
      bold-label bullet lists went to prose, and the sections that announced what the next heading
      already said were deleted rather than reworded.

      **Two things this recorded that are worth keeping.**
      - **The skill catches other people's prose and misses your own.** Four fresh instances of the
        "X rather than Y" shape were written into `CONTRIBUTING.md` *minutes after* reading the rule
        that names it. The author of the text is the worst reviewer of it.
      - **A cut is only safe if you check where the claim now lives.** Every removal here was
        grepped for in `docs/`, `website/docs/` and the nav before it was made, which is the
        difference between shortening and losing.
