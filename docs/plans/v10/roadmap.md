...
| ~~**Nothing observable at runtime**: no round-trip counter~~ ✅ **done 2026-09-07.** A `Meter` named `InfoCarrier.Core` publishes a count and a duration histogram per round trip, tagged by operation and by exception type on a failure, measured in the one method all nine operations funnel through. The remaining half of the original item, a logger category of this provider's own, is a decision and not a gap: this provider's events go under EF's `Microsoft.EntityFrameworkCore.Query`, as every EF provider's do | ~~post-v10~~ done |
...
| **`InfoCarrier.Core.Relational`, a companion package on BOTH halves.** Raised 2026-09-02 by the owner. `InfoCarrier.Core` references nothing relational (§6a D3), so every relational fact it needs is a magic string or a reflection read: 9 `Relational:` annotation strings pinned by a 268-line test, `RelationalQueryRootShape`'s 276 lines and 10 trim suppressions, a hand-written narrower copy of EF's `EntityTypeHierarchyMappingConvention`. A second package referenced by client and server deletes them **without reversing D3**, because the reference lives outside `InfoCarrier.Core`. Measured, priced in three levels and tracked as [#97](https://github.com/azabluda/InfoCarrier.Core/issues/97). **LEVEL 1 IS BUILT (R120, 2026-09-02)**: the package ships, `RelationalQueryRootShape` is gone, and §6a D3's R120 amendment records the seam and the one-reader constraint it uncovered. **LEVEL 2 IS ANSWERED AND THE ANSWER IS "ONLY THE FIX-UP ONES" (R170, 2026-09-04)**: EF's whole relational convention list was tried on the client and cost 681 tests, so the client runs the four conventions that MOVE an annotation when EF replaces the type it hangs off, and none of the ones that DECIDE a name, a shape or a strategy, because the server decides those with a provider the client cannot see. **LEVEL 3 IS DONE (V5, 2026-09-07)**: the client builds a relational model. It needed EF's two table-name conventions (both read the caller's own code, so the two halves cannot disagree), `RelationalRuntimeModelConvention` to carry the model across the runtime conversion, EF's real annotation provider in place of the throwing stub, and EF's relational code generator in the design-time services. The store type names are EF's own neutral ones and name no database | levels 1, 2 and 3 done |
