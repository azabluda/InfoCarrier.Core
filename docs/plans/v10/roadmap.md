...
| **`InfoCarrier.Core.Relational`, a companion package on BOTH halves.** Raised 2026-09-02 by the owner. `InfoCarrier.Core` references nothing relational (§6a D3), so every relational fact it needs is a magic string or a reflection read: 9 `Relational:` annotation strings pinned by a 268-line test, `RelationalQueryRootShape`'s 276 lines and 10 trim suppressions, a hand-written narrower copy of EF's `EntityTypeHierarchyMappingConvention`. A second package referenced by client and server would delete them **without reversing D3**, because the reference lives outside `InfoCarrier.Core`. Measured, priced in three levels and tracked as [#97](https://github.com/azabluda/InfoCarrier.Core/issues/97); **nothing is decided and no design exists** | future scope |
