...
- [ ] **B6. `ValueGenerated` is part of the shared model and the client does not compute it.**
      ...
      the client while the server has `OnAdd`/`OnAddOrUpdate`. Worth 8 in `StoreGenerated` alone.

      **It is a decision about what the client's model is for, so do not start it without asking.**
      The premise of the wire is that both sides build the same model, which argues the client
      should honour the annotations; the client is deliberately store-agnostic and has no business
      knowing what a column default is, which argues the opposite. A third route — have the client
      accept any store-generated value the server actually returns, rather than deciding in advance
      which properties may receive one — changes no model and may be the smallest correct answer.
