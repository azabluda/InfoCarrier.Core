...
- [x] **C93. The compiled-model baselines exist, and `Scaffolding.CompiledModel` is CLOSED.**
      **`Total tests: 22455, Passed: 22215, Failed: 22, Skipped: 218`** (`c93`) — **26 → 22,
      4 fixed, 0 broken.** ✅ `<this commit>`

      Forty-two files under `test/InfoCarrier.Core.FunctionalTests/Scaffolding/Baselines/`,
      written by EF's own mechanism — `EF_TEST_REWRITE_BASELINES=1` — which is how every EF
      provider maintains its own. Nothing is skipped and no assertion is inverted: this supplies
      the expected output the base compares against, and the four tests had already been *seen*
      to fail against it (`Differences found in 17 files`) through C90, C91 and C92.

      **`<Compile Remove="Scaffolding\Baselines\**\*" />`, and it is not optional.** The baselines
      are expected output rather than source, and the SDK globs them in: the same
      `TestNamespace.DbContextModel` is generated once per test, so building them gave **125**
      duplicate-definition errors. EF's InMemory and Sqlite test projects carry that line and its
      `<None Include>` partner verbatim; taking one without the other is what produced the 125.

      **What the four now pin, which is the reason to keep them rather than to have got them
      green.** They are the only assertion in the suite over the *source* this provider
      contributes to a compiled model — `InfoCarrierTypeMapping.Default.Clone(…)` on every
      property, and the `DesignTimeProviderServices` attribute that lets a model be generated at
      all. A change to either is now a diff rather than a discovery.

      **Where C8 and C0 each ended up.** C8 was wrong twice — about the package (C90) and about
      `AssertBaseline` returning early (it returns early on a missing *test source* directory, not
      a missing baselines one, and it creates the latter). C0 was right in kind about the file
      count and wrong about what it cost: 42 files, generated by one command, after the three real
      obstacles behind them were closed.

- [x] **C94. EF's own two upstream skips, adopted — and the fact that settles it is that both are
      SQLite's.**
      **`Total tests: 22453, Passed: 22215, Failed: 17, Skipped: 221`** (`c94`) — **22 → 17,
      5 fixed, 0 broken.** ✅ `<this commit>`

      Declined three times before, on the reasonable worry that skipping a test EF only skips on
      one store would hide a real failure on another. **Grep settles it**: `Issue#28886` appears
      only in `NorthwindBulkUpdatesSqliteTest` and `Issue #30730`'s
      *"SQLite is not matching elements here"* only in
      `NonSharedPrimitiveCollectionsQuerySqliteTest`. SQL Server skips neither, under those
      reasons. Both of our classes are **Tier B**, so adopting the attributes there loses no Tier
      C coverage when M7 lands — a Tier C class would be a different class and would inherit
      neither.

      This is A63 — adopting EF's *own* override where the reason matches — and not the
      suppression CLAUDE.md forbids. The reasons match exactly: the SQL EF records under #28886 is
      the truncated `UPDATE` this provider produces character for character (`no such column:
      c.CustomerID` / `o.OrderID`), and C62 had already reached #30730's conclusion independently
      by reading the row the store actually holds.

      **`total` goes 22455 → 22453, and that is the interesting part.** xUnit reports a **skipped
      theory as one test** rather than as its parameterizations, so the two `[ConditionalTheory]`
      skips turn 4 tests into 2. Nothing stopped running. `test/known-failures.txt` is lowered
      deliberately with that reason written next to it, because a shrinking total is otherwise
      exactly the crashed-host signal the ratchet's second guard exists to catch.
